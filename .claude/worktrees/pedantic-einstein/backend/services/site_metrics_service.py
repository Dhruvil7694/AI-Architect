...
def _effective_cop_area_sqm(plot_area_sqm: float) -> float:
    """
    Required COP area per GDCR: max(required_fraction × plot_area, minimum_total_area_sqm).
    Matches the envelope/common_plot_carver logic so UI shows the same % as the plan.
    """
    try:
